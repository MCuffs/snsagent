...
export default async function AdminUserDetailPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  return (
    <>
      <AdminPageHeader title={user.email} description={`가입일 ${formatDate(user.createdAt)} · 플랜 ${user.plan}`} />
        ...
        <div className="space-y-5">
          <Section title="기본 정보">
            <dl className="grid gap-3 text-sm sm:grid-cols-2">
              ...
              <Info label="NicePay 상태" value={user.nicepaySubscriptionStatus || '-'} />
              <Info label="PayPal 상태" value={user.paypalSubscriptionStatus || '-'} />
            </dl>
          </Section>
            ...
            {user.paymentRecords.length === 0 ? <EmptyState>결제 내역이 없습니다.</EmptyState> : (
              <div className="overflow-x-auto">
                <table className="w-full min-w-[620px] text-left">
                  <thead className="border-b border-[#f0f0f0]">
                    <tr><Th>주문 ID</Th><Th>금액</Th><Th>상태</Th><Th>결제일</Th><Th>환불일</Th></tr>
                  </thead>
                  <tbody className="divide-y divide-[#f5f5f5]">
        ...
        <div className="space-y-5">
          ...
          <Section title="수동 결제 기록">
            <form action={createManualPaymentRecordAction} className="space-y-2">
              ...
              <button className={`${btnCls} w-full`}>결제 기록 생성</button>
            </form>
          </Section>
