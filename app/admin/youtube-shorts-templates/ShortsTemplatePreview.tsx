...
export function ShortsTemplatePreview({ template, compact = false }: {
  ...
  const { layout, headerStyle, captionStyle, cta } = template.config
    ...
    <div className={`relative overflow-hidden rounded-lg border shadow-sm ${compact ? 'h-40 w-[90px]' : 'aspect-[9/16] w-full max-w-[320px]'}`} style={{ backgroundColor: layout.backgroundColor }}>
      {layout.headerEnabled && (
        <div className="flex items-center px-[6%]" style={{
          ...
          backgroundColor: headerStyle.headerBackgroundColor,
          color: headerStyle.headerTextColor,
          justifyContent: headerStyle.headerTextAlign === 'left' ? 'flex-start' : headerStyle.headerTextAlign === 'right' ? 'flex-end' : 'center',
        }}>
          <span className={`${compact ? 'text-[5px]' : 'text-base'} font-black leading-tight`}>시선을 잡는 첫 문장</span>
        </div>
      )}
