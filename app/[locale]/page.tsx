...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    ...
    description: isEn
      ? 'Create video card news and image card news from a URL and topic. AI plans the story, writes copy, prepares Kling video prompts, creates visuals, and exports social-ready content.'
      : 'URL과 주제를 입력하면 AI가 영상 카드뉴스 기획, 카피, Kling 영상 프롬프트, 이미지 카드뉴스 비주얼을 자동 생성합니다.',
    alternates: {
    ...
    openGraph: {
      title: isEn ? 'Shuffla — AI Video Card News Generator' : 'Shuffla — AI 영상 카드뉴스 생성기',
      description: isEn
        ? 'Generate connected video card news scripts, Kling video prompts, and social-ready card news from one brief.'
        : '하나의 주제로 연결감 있는 영상 카드뉴스 기획, Kling 프롬프트, 카드뉴스 콘텐츠를 생성합니다.',
      url: `${base}/${locale}`,
    ...
    twitter: {
      ...
      title: isEn ? 'Shuffla — AI Video Card News Generator' : 'Shuffla — AI 영상 카드뉴스 생성기',
      description: isEn
        ? 'Create video card news, Kling prompts, AI copy, visuals, and social-ready exports.'
        : '영상 카드뉴스, Kling 프롬프트, AI 카피, 비주얼을 한 번에 생성합니다.',
      images: [`${base}/og-image.png`],
      site: '@shuffla_io',
    },
    keywords: isEn
      ? ['AI video card news generator', 'video card news maker', 'Kling video prompt generator', 'AI card news generator', 'Instagram carousel video', 'short form content generator', 'social media content SaaS']
      : ['AI 영상 카드뉴스 생성기', '영상 카드뉴스 만들기', 'Kling 영상 프롬프트', '카드뉴스 자동 생성', 'AI 카드뉴스', '숏폼 콘텐츠 생성', 'SNS 콘텐츠 자동화', '셔플라'],
  }
}
    ...
    '@graph': [
      {
        ...
        url: `${base}/${locale}`,
        description: isEn
          ? 'AI video card news generator: enter a URL and topic to generate connected video card news planning, copy, Kling video prompts, image card news visuals, and social-ready exports.'
          : 'URL과 주제를 입력하면 연결감 있는 영상 카드뉴스 기획, 카피, Kling 영상 프롬프트, 이미지 카드뉴스 비주얼을 생성하는 AI SaaS.',
        offers: {
          '@type': 'Offer',
          price: '0',
          priceCurrency: 'KRW',
        ...
        featureList: isEn
          ? ['Video card news story planning', 'Kling video prompt generation', 'Brand URL analysis', 'AI copy generation', 'AI background image generation', '4:5 Instagram card news export', 'Card news editing studio']
          : ['영상 카드뉴스 스토리 기획', 'Kling 영상 프롬프트 생성', '브랜드 URL 분석', 'AI 카피 자동 생성', 'AI 배경 이미지 생성', '4:5 인스타그램 카드뉴스 내보내기', '카드뉴스 편집 스튜디오'],
        knowsAbout: isEn
          ? ['video card news', 'Kling video generation', 'Instagram carousel', 'AI social content', 'card news design']
          : ['영상 카드뉴스', 'Kling 영상 생성', '인스타그램 카드뉴스', 'AI SNS 콘텐츠', '카드뉴스 디자인'],
        slogan: isEn ? 'From topic to video card news in minutes' : '주제에서 영상 카드뉴스까지 몇 분 안에',
      },
      {
        '@type': 'FAQPage',
        mainEntity: isEn ? [
          { '@type': 'Question', name: 'What is Shuffla?', acceptedAnswer: { '@type': 'Answer', text: 'Shuffla is an AI video card news and card news generator. Enter a brand URL and topic — AI plans the storyline, writes copy, prepares Kling video prompts, creates visuals, and helps export social-ready content.' } },
          { '@type': 'Question', name: 'Can Shuffla generate video card news?', acceptedAnswer: { '@type': 'Answer', text: 'Yes. Shuffla creates editable video card news plans with connected scene prompts designed for Kling video generation, plus scripts and slide copy for each card.' } },
          ...
          { '@type': 'Question', name: 'Shuffla란 무엇인가요?', acceptedAnswer: { '@type': 'Answer', text: 'Shuffla는 AI 영상 카드뉴스 및 카드뉴스 생성 SaaS입니다. URL과 주제를 입력하면 AI가 스토리라인, 카피, Kling 영상 프롬프트, 비주얼을 생성합니다.' } },
          { '@type': 'Question', name: '영상 카드뉴스를 만들 수 있나요?', acceptedAnswer: { '@type': 'Answer', text: '네. Shuffla는 Kling 영상 생성에 맞춘 연결형 장면 프롬프트, 슬라이드별 스크립트, 제목과 본문을 생성하고 사용자가 직접 수정할 수 있게 합니다.' } },
