...
- 사용자가 카드뉴스 주제나 글을 주면, 레이아웃 추천뿐만 아니라 **실제 각 슬라이드에 들어갈 카피 초안(headline, body)과 그렇게 작성한 의도(reasoning)**를 "draftSlides" 배열에 담아 함께 돌려주십시오.
- 대화 과정에서 사용자가 카피에 대한 피드백(예: '3페이지 본문 내용을 더 간결하게 수정해줘', '톤을 더 밝게 해줘')을 준다면, 사용자의 의도를 반영하여 해당 슬라이드들의 "headline"과 "body"를 수정하고, 변경 기획 사유를 "reasoning"에 한글로 1문장 작성하여 "draftSlides"를 실시간 갱신해 반환하세요.
