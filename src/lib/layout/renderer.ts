...
async function renderArchiveCta(input: RenderMediaCardInput) {
  ...
  if (bodyLines.length > 4) {
    throw new Error(`CTA layout body exceeds renderable line limit (${bodyLines.length}/4)`)
  }
...
<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1350" viewBox="0 0 1080 1350">
  ...
  ${bodyLines.map((line, index) => `<text xml:space="preserve" x="540" y="${1002 + index * (typography.bodyFontSize + 14)}" text-anchor="middle" font-family="${fontFam}" font-size="${typography.bodyFontSize}" font-weight="400" fill="${secondaryTextColor}" fill-opacity="0.64" letter-spacing="-0.2">${escapeXml(line)}</text>`).join('')}
